...
impl<'a, B: UsbBus> InterfaceClass<'a> for RawInterface<'a, B> {
    ...
    fn set_report(&mut self, data: &[u8]) -> usb_device::Result<()> {
        ...
        if out_buffer.is_empty() {
            if out_buffer.extend_from_slice(data).is_ok() {
                crate::trace!("Set report, {:X} bytes", &out_buffer.len());
                Ok(())
            } else {
                crate::error!(
            ...
            crate::trace!("Failed to set report, buffer not empty");
    ...
    fn get_report(&mut self, data: &mut [u8]) -> usb_device::Result<usize> {
        ...
        if in_buffer.is_empty() {
            crate::trace!("GetReport would block, empty buffer");
            Err(UsbError::WouldBlock)
        } else if data.len() < in_buffer.len() {
            crate::error!("GetReport failed, buffer too short");
            Err(UsbError::BufferOverflow)
        } else {
    ...
    fn get_report_ack(&mut self) -> usb_device::Result<()> {
        ...
        if in_buffer.is_empty() {
            crate::error!("GetReport ACK failed, empty buffer");
            Err(UsbError::WouldBlock)
        } else {
            in_buffer.clear();
            Ok(())
    ...
    fn set_protocol(&mut self, protocol: HidProtocol) {
        ...
        crate::info!("Set protocol to {:?}", protocol);
    }
